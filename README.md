...
대규모 언어 모델(LLM)은 의료 질의응답에 활용될 수 있으며, 다양한 의료 정보를 자연어 형태로 제공할 수 있다. 다만 의료 분야에서는 답변의 정확성뿐만 아니라 답변에 활용된 정보의 근거와 의료 개념 간의 관계를 함께 확인할 수 있는 환경이 중요하다.

일반적인 RAG 방식은 관련 문서를 검색하여 LLM의 답변 생성에 활용할 수 있지만, 의료 질문에 포함된 개념 간의 구조적인 관계나 임상 진료지침과 같은 전문적인 근거 정보를 함께 활용하기 위해서는 추가적인 검색 및 연계 과정이 필요하다.

이에 본 프로젝트에서는 질문에 포함된 의료 개념을 식별하고, 의료 지식 그래프의 구조적 관계 정보와 임상 진료지침을 함께 검색하여 답변 생성에 활용하는 GraphRAG 기반 의료 질의응답 시스템 MedicalQA를 개발하였다.

이를 통해 LLM의 생성 능력에만 의존하지 않고, 의료 지식 그래프와 임상 진료지침을 기반으로 근거 정보를 함께 제공하는 의료 질의응답 서비스를 구현하고자 하였다.
